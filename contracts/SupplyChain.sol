...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier forSale(uint _sku) {
    require( _sku < skuCount && items[_sku].state == State.ForSale );
    _;
  }
  modifier sold (uint _sku) {
  require( _sku < skuCount && items[_sku].state == State.Sold );
  _;
  }
  modifier shipped (uint _sku) {
  require( _sku < skuCount && items[_sku].state == State.Shipped );
  _;
  }
  modifier received (uint _sku) {
  require( _sku < skuCount && items[_sku].state == State.Received );
  _;
  }
  ...
  function buyItem(uint sku)
    public payable forSale(sku)/*forSale(sku) paidEnough(items[sku].price) checkValue(sku)*/
  {
